...
public class GameOfLife {
	...
	public static void main(String[] args) {
		...
		// System.out.println(Arrays.deepToString(read(fileName)));
		// read("./line.dat");
	...
	public static int[][] evolve(int[][] board) {
		...
		int[][] nextGerationBoard = new int[rows][cols];

		for(int i=1;i<rows-1;i++){
			for(int j=1;j<cols-1;j++){
				nextGerationBoard[i][j] = cellValue(board, i, j);
			}
		}
		return nextGerationBoard;
	}
